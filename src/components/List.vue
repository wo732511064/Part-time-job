<template>
	<div>
		<div class="index-list-header">
			...
			<div class="index-list-header-frame" @click.prevent.stop="listShow()" @keydown.prevent.40="arrowDown1" @keydown.prevent.38="arrowUp1">
				{{selectItem}}
			...
			<div class="index-list-header-frame" @click.prevent.stop="listShow2()" @keydown.prevent.40="arrowDown2" @keydown.prevent.38="arrowUp2">
				{{selectItem2}}
			...
			<div class="index-list-header-frame" @click.prevent.stop="listShow3()" @keydown.prevent.40="arrowDown3" @keydown.prevent.38="arrowUp3">
				{{selectItem3}}
		...
		<div style="position: relative;">
			...
			<div class="index-list-frame" ref="foodsWrapper" v-show="list">
				...
				<div class="index-list-frame-tishi1" v-show="shangla">上拉加载更多</div>
				<div class="index-list-frame-tishi" v-show="tishi">没有更多了...</div>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			menu() {
				...
				if(document.documentElement.offsetHeight == document.documentElement.clientHeight + this.scroll) {
					_this.offset += 20;
					this.$post('parttime/item-list',{
								offset: _this.offset,
								jobtype: _this.gongzuoid,
								recruittype: _this.zhaomuid,
								settlementtype: _this.jiesuanid
							}
					).then((response) => {
							if(response.status == "success") {
								let data1 = JSON.parse(response.data)
								_this.Lists = _this.Lists.concat(JSON.parse(response.data))
								_this.shangla = true;
								_this.tishi = false;
							} else {
								_this.shangla = false;
			...
			postdata() {
				...
				this.$post('parttime/item-list',
					{
						...
						_this.Lists = JSON.parse(response.data);
						setTimeout(() => {
						 	 _this.loading = false;		
						 }, time)
						_this.list = true;
			    ...
			    this.$post('parttime/filter-type',
			        ...
			        _this.obj = JSON.parse(response.data).jobList;
					_this.obj2 = JSON.parse(response.data).recruitTypeList;
					_this.obj3 = JSON.parse(response.data).settlementType;
			      })
			},
			ListPost(item) {
				let _this = this
		...
		watch: {
			...
			'$route' (to, from) {
				if(to.name === 'List') { //跳转的的页面的名称是"List",
					//获取vuex记录的滚动条位置，当页面返回到首页时回到滚动条位置
